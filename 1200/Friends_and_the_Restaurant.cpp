...
void solve()
{
	...
	loop(i, 0, n - 1) {
		if (b[i] >= a[i])ex.insert(b[i] - a[i]);
		else lc.insert(a[i] - b[i]);
	}
	debug(ex);
	debug(lc);
	int ans = 0;
	while (!ex.empty() && !lc.empty()) {
		int extra = *ex.rbegin();
		int lack = *lc.rbegin();
		while (!lc.empty() && lack > extra)
		{
			lc.erase(lc.find(lack));
			if (!lc.empty())
				lack = *lc.rbegin();
		}
		if (lc.empty())break;
		ans++;
		ex.erase(ex.find(extra));
		lc.erase(lc.find(lack));

	}
	debug(ex);
	debug(lc);
	cout << ans + (ex.size() / 2) << nl;
}
int main()
{
	init_code();
