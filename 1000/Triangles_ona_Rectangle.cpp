...
void solve()
{
	...
	cin >> k1;
	h1.resize(k1);
	loop(i, 0, k1 - 1)cin >> h1[i];
	int k2;
	cin >> k2;
	h2.resize(k2);
	loop(i, 0, k2 - 1)cin >> h2[i];
	int k3;
	cin >> k3;
	v1.resize(k3);
	loop(i, 0, k3 - 1)cin >> v1[i];
	int k4;
	cin >> k4;
	v2.resize(k4);
	loop(i, 0, k4 - 1)cin >> v2[i];
	ll ans = LLONG_MIN;
	ans = max(ans, max(h1[k1 - 1] - h1[0], h2[k2 - 1] - h2[0]) * h);
	ans = max(ans, max(v1[k3 - 1] - v1[0], v2[k4 - 1] - v2[0]) * w);
	cout << ans << nl;
}
int main()
{
