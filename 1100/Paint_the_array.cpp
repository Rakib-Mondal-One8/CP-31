...
/*Problem Link -> https://codeforces.com/contest/1618/submission/274037910*/
...
void solve()
{
	...
	debug(even);
	bool ok = true;
	for (int i = 0; i < n; i += 2) if (a[i] % odd == 0) ok = false;
	if (ok) {
		cout << odd << nl;
		return;
	}
	else ok = true;
	for (int i = 1; i < n; i += 2)if (a[i] % even == 0) ok = false;
	if (!ok)cout << 0 << nl;
	else cout << even << nl;
}
int main()
