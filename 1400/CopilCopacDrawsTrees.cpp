...
void solve()
{
	...
	vector<vector<int>>track(n + 1, vector<int>());
	for (int i = 1; i < n; i++) {
		track[a[i].first].push_back(i);
		track[a[i].second].push_back(i);
	}

	deque<pi>d;
	for (auto x : track[1]) {
		int val = a[x].first + a[x].second - 1;
		d.push_back({val, x});
	}
	...
	while (!d.empty()) {
		...
		int val = cur.first;
		int idx = cur.second;
		usedIndex.insert(idx);

		if (track[val][0] != idx) {
			ans++;
			debug(cur);
		}
		for (auto pos : track[val]) {
			if (usedIndex.find(pos) == usedIndex.end()) {
				int valAtPos = a[pos].first + a[pos].second - val;
				d.push_back({valAtPos, pos});
			}
		}
		d.pop_front();
	}
