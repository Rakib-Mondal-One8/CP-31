...
pll intersection(pll a, pll b) {
	return make_pair(max(a.first, b.first), min(b.second, a.second));
}
bool go(ll x, vector<pll>&a, int n) {
	pll curRange = {0, 0};
	for (int i = 0; i < n; i++) {
		curRange = {curRange.first - x, curRange.second + x};
		pll desRange = a[i];
		pll finalRange = intersection(curRange, desRange);
		if (finalRange.first <= finalRange.second)curRange = finalRange;
		else return false;
	}
	return true;
}
